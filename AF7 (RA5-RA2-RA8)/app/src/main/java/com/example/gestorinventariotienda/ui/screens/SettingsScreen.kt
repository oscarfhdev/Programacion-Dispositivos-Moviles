...
fun SettingsScreen(
    ...
    Scaffold(
        topBar = { TopAppBar(title = { Text("Ajustes y Buzón") }) }
    ) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Text("Configuración de la Tienda", style = MaterialTheme.typography.titleLarge)
            ...
            Button(
                ...
                Text("Redactar Reporte de Incidencia")
            }

            Spacer(modifier = Modifier.height(24.dp))
            Text("Historial de Incidencias", style = MaterialTheme.typography.titleMedium)
            Spacer(modifier = Modifier.height(8.dp))

            // Lista del historial de incidencias que vienen de Room
            if (incidents.isEmpty()) {
                Text("No hay incidencias registradas.", style = MaterialTheme.typography.bodyMedium)
            } else {
                LazyColumn(
